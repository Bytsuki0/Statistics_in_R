...
MIN_CRASH_SEP     <-  31000
MIN_DROP_PCT      <-  -15.0   # queda minima para qualificar como crash (%)
MIN_OBS_AHEAD     <-  50000   # minimo de observacoes apos X para incluir o evento
...
# --- Filtro 1: remove crashes cuja queda diaria foi menor que MIN_DROP_PCT% ---
# Recalcula close_pct para cada crash (queda percentual no dia identificado)
crash_pct_vec <- sapply(seq_along(crash_days), function(i) {
  row_i     <- crash_day_rows[i]
  diff_val  <- daily_close$close_diff[row_i]
  prev_close <- daily_close$close_last[row_i] - diff_val   # close do dia anterior
  if (prev_close == 0 || is.na(prev_close)) return(NA_real_)
  (diff_val / prev_close) * 100   # percentual; negativo = queda
})

keep_pct <- !is.na(crash_pct_vec) & (crash_pct_vec <= MIN_DROP_PCT)
if (any(!keep_pct)) {
  cat(sprintf("  [Filtro DROP_PCT] Removidos %d crash(es) com queda < %.0f%%:\n",
              sum(!keep_pct), abs(MIN_DROP_PCT)))
  for (i in which(!keep_pct))
    cat(sprintf("    -> %s  (queda = %.2f%%)\n", crash_days[i], crash_pct_vec[i]))
}
crash_idx_vec <- crash_idx_vec[keep_pct]
crash_days    <- crash_days[keep_pct]

# --- Filtro 2: remove crashes sem MIN_OBS_AHEAD observacoes no futuro ---
# Necessario para garantir que a janela POST_END_OFFSET nao fique truncada
keep_future <- (n_total - crash_idx_vec) >= MIN_OBS_AHEAD
if (any(!keep_future)) {
  cat(sprintf("  [Filtro OBS_AHEAD] Removidos %d crash(es) perto do fim da serie:\n",
              sum(!keep_future)))
  for (i in which(!keep_future))
    cat(sprintf("    -> idx=%d  obs restantes=%d  (minimo=%d)\n",
                crash_idx_vec[i], n_total - crash_idx_vec[i], MIN_OBS_AHEAD))
}
crash_idx_vec <- crash_idx_vec[keep_future]
crash_days    <- crash_days[keep_future]

n_events <- length(crash_idx_vec)
cat(sprintf("\nEventos validos apos filtros: %d\n", n_events))
if (n_events == 0) stop("Nenhum evento sobreviveu aos filtros. Ajuste MIN_DROP_PCT ou MIN_OBS_AHEAD.")
...
# --- Constroi os janelamentos (n_events x 3 fases) ---
events <- vector("list", n_events)
...
ann_draws_all <- vector("list", n_events)
...
# ============================================================
bp_fit     <- strucchange::breakpoints(ret_daily ~ 1, breaks = 5)
