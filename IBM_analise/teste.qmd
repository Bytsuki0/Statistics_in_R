...
```{r detect-crashes}
# ============================================================
# PARAMETROS DOS JANELAMENTOS
# Para cada crash no indice X:
#   pre   : [X - 20000 , X - 10000]
#   during: [X -  1000 , X +  9000]
#   post  : [X + 20000 , X + 30000]
#
# Separacao minima entre crashes: 31000 observacoes
# (garante que os janelamentos de eventos distintos nao se sobreponham)
# ============================================================
...
# ============================================================
# NOVO CRITERIO DE DETECCAO DE CRASHES
#
...
# --- Passo 1: fechamento diario (ultimo Close de cada dia) ---
daily_close <- dt1 %>%
...
# unname() nos quantis evita que o nome "2.5%" seja concatenado ao prefixo,
# o que geraria colunas "q2.5.2.5%" em vez de "q2.5" e quebraria a selecao posterior
summ_param <- function(parmat) {
  res <- t(apply(parmat, 2, function(x)
    c(mean  = mean(x),
      sd    = sd(x),
      q2.5  = unname(quantile(x, .025)),
      q97.5 = unname(quantile(x, .975)))))
  res <- as.data.frame(res)
  colnames(res) <- c("mean", "sd", "q2.5", "q97.5")
  res
}
