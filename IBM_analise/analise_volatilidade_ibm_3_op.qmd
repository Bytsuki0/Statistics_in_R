...
dc_clean <- daily_close %>%
  filter(!is.na(close_diff), !is.na(close_prev), close_prev > 0) %>%
  mutate(daily_ret = close_diff / close_prev)
...
bp_row_idx <- bp_fit$breakpoints   # indices de linha em dc_clean

crash_idx_vec_2 <- sort(as.integer(Filter(Negate(is.na), sapply(bp_row_idx, function(i) {
  g <- dc_clean$last_global_idx[i]
  if (is.na(g) || (g + MIN_FUTURE_IDX) > n_total) return(NA_integer_)
  as.integer(g)
}))))
...
bp_summary <- data.frame(
  ...
  retained   = dc_clean$last_global_idx[bp_row_idx] %in% crash_idx_vec_2
)

cat("\nBai-Perron — 5 quebras na media dos retornos diarios (crash_idx_vec_2):\n")
print(knitr::kable(bp_summary,
                   col.names = c("Rank", "Data", "Ret diario (%)",
                                 "Idx global", "Retido (dados futuros ok)"),
                   caption = "Bai-Perron: quebras estruturais na media"))
cat("crash_idx_vec_2:", crash_idx_vec_2, "\n")
...
if (length(pelt_row_idx) > 5) {
  w  <- 30L
  dv <- sapply(pelt_row_idx, function(i) {
...
crash_idx_vec_3 <- sort(as.integer(Filter(Negate(is.na), sapply(pelt_row_idx, function(i) {
  g <- dc_clean$last_global_idx[i]
  if (is.na(g) || (g + MIN_FUTURE_IDX) > n_total) return(NA_integer_)
  as.integer(g)
}))))
...
pelt_summary <- data.frame(
  ...
  retained   = dc_clean$last_global_idx[pelt_row_idx] %in% crash_idx_vec_3
)

cat("\nPELT — quebras na variancia dos retornos diarios (crash_idx_vec_3):\n")
print(knitr::kable(pelt_summary,
                   col.names = c("Rank", "Data", "Ret diario (%)",
                                 "Idx global", "Retido (dados futuros ok)"),
                   caption = "PELT (cpt.var): quebras estruturais na variancia"))
cat("crash_idx_vec_3:", crash_idx_vec_3, "\n")

# ============================================================
# Visualizacao comparativa — 3 metodos
# ============================================================
...
all_marks <- bind_rows(
  data.frame(x      = crash_idx_vec,
             ...
             stringsAsFactors = FALSE),
  data.frame(x      = crash_idx_vec_2,
             metodo = "Bai-Perron",
             label  = paste0("BP", seq_along(crash_idx_vec_2)),
             y_pos  = price_max * 0.89,
             stringsAsFactors = FALSE),
  data.frame(x      = crash_idx_vec_3,
             metodo = "PELT",
             label  = paste0("PL", seq_along(crash_idx_vec_3)),
...
for (ev in events) {
  ...
  zoom_s <- max(1,       ev$crash_idx - 32000)
  zoom_e <- min(n_total, ev$crash_idx + 42000)
