...
Em ambos os períodos, a **ACF dos retornos** decai imediatamente para valores próximos de zero após o lag 0.

#### Pré-Queda

-   **ACF**: Queda abrupta após lag 0; todos os coeficientes dentro das bandas de confiança (\~±2/√T).\
-   **PACF**:
    -   **Lag 1**: Leve autocorrelação parcial negativa (\~–0.02).\

    -   **Lags 5–20**: Pequenos “espasmos”, mas sem ultrapassar bandas, indicando ruído aleatório.
-   **Pré-Queda:** Retornos se comportam como ruido branco com leve reversão imediata.

#### Pós-Queda

-   **ACF**: Mesma queda imediata; coeficientes insignificantes após lag 0.\
-   **PACF**:
    -   **Lag 1**: Reversão de preço mais pronunciada (\~–0.03), sinal de reação rápida de compradores da opção.\

    -   **Lags 10–40**: Picos positivos e negativos sugerem padrões de liquidez intradiária possivelmente janelas de execução de algoritmos.
-   1\. **Reversão de curtíssimo prazo** intensificada (lag 1 negativo).\
    2.  **Microestrutura & Liquidez**: agrupamentos de ordens e padrões de execução visíveis em lags mais altos.
