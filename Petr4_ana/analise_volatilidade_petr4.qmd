...
set.seed(123)
if (file.exists("sv_fit1.rds")) {
  sv_fit <- readRDS("sv_fit1.rds")
} else {
  ret_vec <- as.numeric(ret.1min[1:10000])
  sv_fit <- svsample(ret_vec, draws = 3000, burnin = 1000)
  saveRDS(sv_fit, "sv_fit1.rds")
}
...
min = idx_max - 2500
max = idx_max + 7500
if (file.exists("sv_fit2.rds")) {
  sv_fit2 <- readRDS("sv_fit2.rds")
} else {
  ret_vec2 <- as.numeric(ret.1min[min:max])
  sv_fit2 <- svsample(ret_vec2, draws = 3000, burnin = 1000)
  saveRDS(sv_fit2, "sv_fit2.rds")
}
```
...
min2 = max + 12000
max2 = min2 + 10000
if (file.exists("sv_fit3.rds")) {
  sv_fit3 <- readRDS("sv_fit3.rds")
} else {
  ret_vec3 <- as.numeric(ret.1min[min2:max2])
  sv_fit3 <- svsample(ret_vec3, draws = 3000, burnin = 1000)
  saveRDS(sv_fit3, "sv_fit3.rds")
}
```

```{r}
ret_vec_complt <- as.numeric(ret.1min)
sv_fit_complt <- svsample(ret_vec_complt, draws = 3000, burnin = 1000)
...
df_est_complt <- data.frame(time = time_index_complt, vol = vol_est_complt)
...
plot_wavelet_levels_modwt_hist(
  df_rv5,
  levels  = 12,
  df_name = "PETR4"
)
